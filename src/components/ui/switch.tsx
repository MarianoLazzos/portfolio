...
>(({ className, ...props }, ref) => (
  ...
  >
    <SwitchPrimitives.Thumb
      className={cn(
        "pointer-events-none block h-7 w-7 rounded-full bg-background shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-6 data-[state=unchecked]:translate-x-0 flex items-center justify-center"
      )}
    >
      {props?.checked ? <Moon size={14} /> : <Sun size={14} />}
    </SwitchPrimitives.Thumb>
  </SwitchPrimitives.Root>
))
